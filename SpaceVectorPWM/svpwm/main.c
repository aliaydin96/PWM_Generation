...
////////////////////////////////////////////////////////////////////////////////////////////   //
 ...
 double phaseAngle = 0;   // starting angle of sinus                                       //   //
                                                                                          //   //
 float magnitudeModulationRatio = 0.9;//Peaktopeak Value of output sinus waveform     //   //
...
void SineTableGenerator(void){
    ...
    while(i < N){
        sineTable[i] = sin(2 * PI * frequencyModulationRatio * i);
        i = i + 1;
    }
}
...
interrupt void ePWMA_compare_isr(void) {
    ...
    int sector = counter * 5 / 799;
    int constant = N / 6;
    teta = counter * 60 / 799;
    T1 = magnitudeModulationRatio * switchingFrequency * sineTable[constant - teta];
    T2 = magnitudeModulationRatio * switchingFrequency * sineTable[teta];
    T0 = switchingFrequency - T1 - T2;
    switch(sector){
        case  0:
            TA=T0 / 2;
            TB=TA+T1;
